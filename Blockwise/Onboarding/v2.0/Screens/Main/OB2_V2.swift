...
struct OB2_V2: View {
    ...
    var question: String {
        "Have you tried to reduce screen time before?"
    }
    
    let subheadline: String = ""

    let answers: [OBAnswer] = [
        OBAnswer(title: "Yes, didn't stick", points: 1),
        OBAnswer(title: "Yes, briefly worked", points: 1),
        OBAnswer(title: "No, first time", points: 0),
    ]
    ...
    private func action(answer: OBAnswer) {
        ...
        SleepTask.sleep(seconds: vm.nextPageDelay) {
            vm.nextPage(progress: 0.3)
        }
        
        vm.hasTriedBefore = answer.points != 0
        
        // Mixpanel
        AnalyticsService.shared.track("OBV2 > First time?")
    }
