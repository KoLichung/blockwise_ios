...
//  Blockwise
//
//  Created by Ivan Sanna on 31/07/25.
//
...
struct BreakStreakWarning: View {
    @State private var appearAnimation: Bool = false
    @State private var streak: Int = 9
    
    var body: some View {
        VStack(spacing: 32) {
            LottieView(animation: .named("fire"))
                .looping()
                .animationSpeed(appearAnimation ? 0.25 : 1.0)
                .frame(square: appearAnimation ? 88 : 100)
...
//                                .font(.system(size: 24, weight: .semibold))
        ...
        .grayscale(appearAnimation ? 1 : 0)
        .frame(maxWidth: .infinity, maxHeight: .infinity)
        .safeAreaInset(edge: .bottom) {
            VStack(spacing: 24) {
                GlassButton("Lose your streak") {
