...
struct OBQuizFour: View {
    ...
    let options: [OBOption] = [
        ...
        OBOption(emoji: "", label: "Not sure", value: 7),
    ]
    
    let questionNumber: Int = 4
    let question: String = "Do you forget things more often than before?"
...
//                    .lineSpacing(2.0)
    ...
    private func action(option: OBOption) {
        ...
        SleepTask.sleep(seconds: vm.actionDelay) {
            vm.nextStep()
            vm.mixPanelTrack(name: "Quiz 4", properties: [question : selectedOption.label])
        }
    }
}
