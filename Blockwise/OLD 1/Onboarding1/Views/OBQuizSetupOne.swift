    ...
    var body: some View {
        VStack(alignment: .center, spacing: 64) {
            ...
            VStack(alignment: .center, spacing: 32) {
                LottieView(animation: .named("paw-prints"))
                    .looping()
                    .animationSpeed(0.8)
                    .frame(square: 150)
                ...
                Text("Let's take the first steps together")
...
//                    .font(.title3.weight(.regular))
    ...
    private func action() {
        vm.nextStep()
        vm.currentSetupProgress = .two
        vm.mixPanelTrack(name: "Quiz Setup 1")
    }
}
