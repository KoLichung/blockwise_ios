...
struct OBQuizTen: View {
    ...
    let options: [OBOption] = [
        ...
        OBOption(emoji: "nerd-face", label: "Never", value: 2)
    ]
    
    let descriptions: [String] = [
        "I usually feel disconnected",
        "It depends on the day",
        "I always feel mentally clear"
    ]
    
    let questionNumber: Int = 10
    let question: String = "Do you find it hard to think clearly?"
...
//                                            .font(.subheadline)
    ...
    private func action(option: OBOption) {
        ...
        SleepTask.sleep(seconds: vm.actionDelay) {
            vm.nextStep()
            vm.mixPanelTrack(name: "Quiz 10", properties: [question : selectedOption.label])
        }
    }
}
