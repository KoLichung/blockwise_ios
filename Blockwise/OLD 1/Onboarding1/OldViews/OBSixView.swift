...
struct OBSixView: View {
    @EnvironmentObject var vm: OBUserViewModel
    
    @State private var selectedOptions: [OBOption] = []
    
    let question = "When do you use your phone the most?"
    let subtitle = "Question #6"
    
    let options: [OBOption] = [
        OBOption(emoji: "💼", label: "At work/school", value: 2),
        OBOption(emoji: "🍽️", label: "During meals", value: 5),
        OBOption(emoji: "🛏️", label: "Right before bed", value: 4),
        OBOption(emoji: "😐", label: "When bored", value: 3)
    ]
